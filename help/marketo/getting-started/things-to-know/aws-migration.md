...
새 날짜 및 데이터 센터/pod 정보가 주기적으로 추가되거나 변경되므로 이 일정을 모니터링하여 업데이트를 진행하십시오. 마이그레이션을 다른 날짜로 이동할지 여부를 옵트아웃하거나 요청할 수 있는 옵션이 없습니다.

어떤 이유로든 마이그레이션에 실패하는 경우 알림 메시지가 표시되고 일정이 조정됩니다.
...
상태 업데이트의 경우 마이그레이션 시작 및 완료 시 [구독을 통해 &#x200B;](https://experienceleague.adobe.com/ko/docs/marketo/using/getting-started/things-to-know/system-status-notifications){target="_blank"}받을 수 있습니다. 마이그레이션 기간 동안 [https://status.adobe.com/ko-kr/](https://status.adobe.com/ko-kr/){target="_blank"}을(를) 방문할 수도 있습니다.
