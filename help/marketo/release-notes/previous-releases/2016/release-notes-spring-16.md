...
다음 기능은 16년 봄 릴리스에 포함되어 있습니다. 각 기능에 대한 자세한 문서를 보려면 제목 링크를 클릭하십시오.

## [이메일 통찰력](/help/marketo/product-docs/reporting/email-insights/email-insights-overview.md) {#email-insights}

이메일 인사이트는 Project Orion의 일부로 빠른 성능을 위해 완전히 재설계된 새로운 기록 집계 데이터 이메일 분석 경험입니다. 이메일 마케터의 요구 사항과 워크플로우에 맞게 최적화된 완전히 새로운 사용자 인터페이스 디자인을 제공합니다.

>[!NOTE]
>
>6월 3일부터 고객에게 이메일 인사이트를 일괄적으로 출시합니다. 우리의 목표는 다음 몇 달 동안 이것을 완료하는 것입니다. 활성화되면 이메일로 알려 드리겠습니다.
...
새로운 시작 템플릿을 사용하여 멋진 이메일을 만들 수 있습니다! 또한 라이브 축소판 그림에서 템플릿을 신속하게 찾을 수 있습니다.
...
## [이메일 편집 - 재이미지](/help/marketo/product-docs/email-marketing/general/email-editor-2/email-editor-v2-0-overview.md) {#email-editing-re-imagined}

맞아요, 새로운 이메일 편집장이에요! 간단한 드래그 앤 드롭 기능을 사용하여 컨텐츠를 추가하거나 순서를 변경할 수 있습니다. 이미지, 비디오, 변수 및 모듈을 포함한 새로운 요소는 편집 경험을 향상시킬 수 있습니다. 업데이트된 코드 편집기, 사전 뷰어 및 사전 헤더 지원을 확인하십시오.
...
Marketo 내에서 바로 앱을 위한 멋진 인앱 메시지를 만들 수 있습니다. 표시되는 사용자와 인앱 메시지 프로그램을 사용할 때를 정확하게 정의합니다. 프로그램 대시보드로 성능을 쉽게 모니터링할 수 있습니다.

![](assets/pasted-image-at-2016-05-24-09-45-am.png)

## [초안 코드 조각 없음](/help/marketo/product-docs/administration/users-and-roles/managing-user-roles-and-permissions/enable-no-draft-for-snippets.md) {#no-draft-snippets}

코드 조각이 업데이트될 때마다 모든 것을 다시 승인해야 하는 시대는 지나갔습니다! 초안이 없는 경우 코드 조각을 사용하는 모든 이메일 및 랜딩 페이지는 코드 조각 업데이트를 받고 이전 상태를 유지 관리합니다. 코드 조각을 승인할 때마다 초안 없음을 실행하고 모든 항목을 업데이트하거나 초안을 만들 수 있습니다. 당신 책임이에요! 초안이 모든 고객이 사용할 수 있으며 관리자의 새 권한에 의해 제어됩니다.
...
이제 Marketo REST API는 Marketo 랜딩 페이지, 랜딩 페이지 템플릿 및 양식에 대한 제어를 지원합니다. 이제 사용자는 Marketo REST API를 통해 직접 이러한 자산을 생성, 업데이트, 승인 및 삭제할 수 있습니다.
...
Orion 아키텍처에 구축된 새로운 고속 Dynamics 커넥터를 사용하면 초기 동기화를 위해 최대 20배 빠른 속도를 제공하고 증분 동기화를 위해 최대 5배 빠른 속도를 제공합니다. 모든 신규 고객은 릴리스 날짜에 이 커넥터에 내장 되며, 기존 고객에게 여름 릴리스 기간 동안 단계적으로 출시될 예정입니다.
...
**동기화 오류 관리 탭**: 작업, 방향, 오류 코드 및 오류 메시지와 같은 세부 정보와 동기화하지 못한 리드(및 기타 개체)를 탐색, 검색 또는 내보냅니다.
...
오늘날 Marketo 인스턴스(예: 샌드박스 및 프로덕션 인스턴스)를 구별하기는 어렵습니다. 이 기능을 사용하면 현재 작업 중인 인스턴스를 알 수 있습니다.
...
오늘날, 사용자들은 무기한 Marketo 구독에 초대된다. 이 기능을 사용하면 관리자가 제한된 기간(예: 2주 또는 1개월) 동안 가입에 사용자를 초대할 수 있습니다.

![](assets/image2016-5-16-15-3a59-3a52.png)

## [사용자 정의 객체 그리드](/help/marketo/product-docs/administration/marketo-custom-objects/understanding-marketo-custom-objects.md) {#custom-objects-grid}
