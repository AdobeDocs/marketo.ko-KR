...
5월 릴리스에서는 대기 흐름 단계에 고급 옵션을 추가했습니다. 이러한 변경 사항으로 업무일, 날짜 및 시간을 지정할 수 있습니다. 이 릴리스에서는 대기 단계에서 토큰을 사용하는 기능을 추가했습니다. 예를 들어 `{{lead.Birthday}}` 을 사용하여 생일에 이메일을 보내거나 `{{my.Event Date}}` 를 사용하여 최종 웨비나 미리 알림을 보낼 수 있습니다.
