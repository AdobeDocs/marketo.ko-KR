...
이제 [사용하지 않은 작업 공간을 삭제](/help/marketo/product-docs/administration/workspaces-and-person-partitions/delete-a-workspace.md)할 수 있습니다. 작업 공간을 삭제하기 전에 모든 자산을 다른 작업 공간으로 이동해야 합니다.

## 첫 번째 캐스트 예약 {#schedule-first-cast}

참여 프로그램에서 [첫 번째 캐스팅이](/help/marketo/product-docs/email-marketing/drip-nurturing/engagement-program-streams/set-stream-cadence.md)실행되도록 날짜를 예약할 수 있습니다. 예를 들어 2주마다 캐덴스를 지정하고 첫 번째 캐스트의 날짜를 선택합니다.
...
[링크가 리디렉션된 Marketo 추적 ](/help/marketo/product-docs/email-marketing/general/functions-in-the-editor/add-tracked-links-to-a-text-email.md) 링크로 변환되어야 하는 시기를 나타내기 위해 이메일의 텍스트 버전에서 URL 주위에 이중 대괄호를 추가합니다
...
* **텍스트 버전:** URL을 입력합니다. 기본적으로 추적되지 않습니다.
...
당황하지 마! 오류가 발생하면 새 [campaign](/help/marketo/product-docs/core-marketo-concepts/smart-campaigns/using-smart-campaigns/abort-a-smart-campaign.md) 버튼을 사용하여 트랙에서 캠페인을 즉시 중지할 수 있습니다. 캠페인이 중지되었을 때 각 흐름 단계에서 보류 중인 리드 수에 대한 개요를 제공하는 알림을 받게 됩니다.
...
프로그램 성공 날짜를 포함하여 각 프로그램 상태](/help/marketo/product-docs/reporting/revenue-cycle-analytics/program-analytics/build-a-program-membership-analysis-report-that-lists-leads.md)에 있는 멤버의 수와 각 상태로 변경된 시간을 확인합니다.[
...
[모바일 ](/help/marketo/product-docs/reporting/basic-reporting/report-activity/build-a-people-performance-report-with-mobile-platform-columns.md) 장치에서 리드의 열기 및 클릭을 세그먼트화하고 트리거합니다.
