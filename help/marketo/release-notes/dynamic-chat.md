...
## 2026년 3월 릴리스 {#march-2026-release}
...
방문자의 이메일 도메인이 알려진 계정에 매핑되는 경우 매핑된 에이전트가 항상 우선적으로 선택됩니다.
...
첫 번째 지원 가능한 에이전트가 참여하지 않으면 시스템은 동일한 목록에서 에이전트 한 명을 더 시도합니다.
...
라운드 로빈이 대체로 사용되는 경우 기본 규칙에서 확인된 에이전트가 없는 경우에만 활성화됩니다.
...
* 에이전트가 적격하지 않거나 사용할 수 없는 경우 카드 수준 라우팅 규칙으로 진행합니다.
...
<tbody>
  ...
  <tr>
    <td>대체 (라운드 로빈)</td>
    <td>지원 가능한 팀 에이전트가 없으며 라운드 로빈 대체가 활성화되어 있습니다.</td>
    <td>시스템이 라운드 로빈 풀에서 한 명의 에이전트를 선택하여 연결합니다.</td>
  </tr>
  <tr>
    <td>대체 에이전트 없음</td>
    <td>두 명의 지원 가능한 에이전트가 있지만 둘 다 참여하지 않으며, 캘린더 대체가 활성화되어 있습니다.</td>
...
<tbody>
  <tr>
    <td>적합</td>
    <td>라운드 로빈 풀에는 여러 에이전트가 있으며, 첫 번째 에이전트가 응답하지 않으면 두 번째 에이전트가 채팅을 수락합니다.</td>
    <td>채팅이 두 번째 에이전트에게 연결됩니다.</td>
  </tr>
  <tr>
    <td>대체 (라운드 로빈)</td>
    <td>라운드 로빈 풀에 지원 가능한 에이전트가 없으며, 회의 캘린더가 활성화되어 있습니다.</td>
    <td>목록의 첫 번째 에이전트(구성된 경우)에 대한 캘린더가 표시되거나 대체 메시지가 표시됩니다.</td>
  </tr>
  <tr>
    <td>대체 에이전트 없음</td>
...
사용자 정의 규칙/팀을 라이브 채팅 카드로 사용하는 경우, 사용 가능한 에이전트가 없거나 채팅에 연결할 수 없는 경우 사용 가능한 에이전트(스트림에 설정된 라우팅 논리/규칙에 관계없이 해당 시간에 이용 가능한 모든 에이전트)에 대해 라운드 로빈으로 대체됩니다.
...
* 평균 에이전트 응답 시간(초): 에이전트가 라이브 채팅 Q&amp;A에 응답하는 데 걸리는 “평균 시간” 분석
...
<tbody>
  ...
  <tr>
    ...
    <td>xyz</td>
    <td>기존 사용자와 다름</td>
    <td>이 시나리오는 기본적으로 새 익명 프로필로 간주되는 새 쿠키인 것처럼 수행할 수 없습니다</td>
  </tr>
</tbody></table>

### 글꼴 상속 옵션 {#option-to-inherit-font}

이제 Dynamic Chat에서 브랜드 글꼴을 관리하는 대신 챗봇이 호스팅 중인 웹 페이지에서 글꼴을 직접 상속할 수 있습니다. 이 옵션을 활성화하면 챗봇이 페이지의 `<body>` 태그에 정의된 글꼴을 가져옵니다.
...
사용자 경험을 개선하기 위해 대화 흐름이 로드되는 동안 공백 대신 쉬머 로더가 표시됩니다.
...
이제 챗봇은 다중 대화를 지원합니다. 웹 사이트 방문자는 서로 다른 페이지에서 동시에 여러 대화에 참여할 수 있으며, 그 대화들 사이를 전환할 수도 있습니다.
...
웹사이트의 대다수 대화는 익명 방문자와 함께 진행됩니다. 방문자 IP에 의존하는 추론 데이터를 통해 여전히 이들을 타기팅할 수 있습니다. IP와 그에 따른 추론 데이터의 데이터베이스를 업데이트했으며, 이제 4배 더 많은 IP를 지원합니다.
...
예: 다중 대화를 통해 웨비나 등록을 유도하는 것이 목표인 경우, 해당 목표를 가진 모든 대화에 걸쳐 동일한 플로우를 다시 만들어야 했습니다. 그리고 세부 사항을 업데이트해야 하는 경우 각 개별 대화를 한 번에 하나씩 편집해야 했습니다. 대화 흐름 카드 덕분에 더 이상 이러한 작업이 필요하지 않습니다.
...
2023년에 대화용 라이브 채팅 기능을 릴리스했으며, 이제 대화 플로우에도 라이브 채팅 참여를 추가할 수 있습니다. Marketo Engage 양식과 함께 대화형 흐름을 사용하는 경우, 이제 양식 제출 직후 자격 있는 방문자가 라이브 에이전트와 채팅하도록 허용할 수 있습니다.
...
### 에이전트 캘린더 구성의 최소 사전 공지 설정 {#minimum-notice-setting-in-agent-calendar-configuration}
...
개별 대화 및 대화 흐름 수준 보고서의 성능과 정확도가 향상되었습니다. 이전에는 대화 보고서를 로드하는 데 몇 초가 걸리고 데이터가 글로벌 성과 보고서와 일치하지 않는 경우가 있었습니다. 이제 개별 대화 보고서가 즉시 로드되고 데이터가 항상 글로벌 보고 데이터와 일치합니다.
...
조건부 분기를 사용하면 다른 조건을 기반으로 대화 플로우에 분기를 만들 수 있습니다. 이제 Marketo Engage의 리드 및 회사 속성을 기반으로 동일한 대화에서 다양한 사용자에게 다양한 콘텐츠를 제공할 수 있습니다.
