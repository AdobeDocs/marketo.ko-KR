...
>[!NOTE]
>
>* 활동 세부 정보 사용자 지정에서 `{{activity_subject}}` 동적 필드를 사용하는 경우 미리 알림 작업의 [!DNL Sales Connect]에 있는 제목 필드에 대한 업데이트는 해당 [!DNL Salesforce] 작업의 제목 필드에 반영됩니다.
...
1. 활동 세부 사항 사용자 지정 편집기에서 원하는 자유 텍스트를 추가합니다. 추가한 텍스트는 비동적이며 [!DNL Salesforce]에 동기화된 모든 작업의 제목 필드에 대해 변경되지 않습니다.
   ...
   >[!TIP]
   >
   >필수는 아니지만, 추가된 텍스트를 대괄호로 묶으면 Salesforce의 제목 필드에 데이터를 채울 때 일부 사용자가 데이터를 보다 쉽게 구분할 수 있습니다. 예: `[Sales Connect] - {{Activity_type}}`
