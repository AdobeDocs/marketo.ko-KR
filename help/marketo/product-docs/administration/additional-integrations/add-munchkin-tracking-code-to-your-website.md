...
>[!PREREQUISITES]
>
>숙련된 JavaScript 개발자에게 액세스할 수 있어야 합니다. Marketo 기술 지원이 사용자 지정 JavaScript 문제 해결을 지원하도록 설정되지 않았습니다.
...
1. JavaScript 추적 코드를 복사하여 웹 사이트에 추가합니다.
...
1. 추적할 웹 페이지에 적합한 작업 영역을 선택합니다.
   ...
   >[!NOTE]
   >
   >특수 작업 영역 [!DNL Munchkin] 코드를 사용하지 않으면 계정 설정 시 생성된 기본 파티션에 사용자가 할당됩니다. 기본적으로 이름이 &#39;[!UICONTROL Default]&#39;이지만 이 이름이 변경되었을 수 있습니다.
...
1. `<head>` 요소의 웹 페이지에 추적 코드를 배치합니다. 이 페이지를 방문하는 새 사용자가 이 파티션에 할당됩니다.
   ...
   >[!NOTE]
   >
   >Marketo에서 만든 랜딩 페이지에는 추적 코드가 자동으로 포함됩니다. 이 코드를 추가할 필요는 없습니다.
...
1. **[!UICONTROL Web Page Activity]**&#x200B;를 클릭합니다.
