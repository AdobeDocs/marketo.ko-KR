...
description: 관리자 로그인 설정을 통해 SSO 보안을 우회할 수 없도록 모든 사용자에게 SSO가 필요합니다(Adobe IMS에는 적용되지 않음).
...
>[!TIP]
>
>**[!UICONTROL Require SSO]**&#x200B;을(를) 선택하는 경우 역할을 설정하는 동안 **[!UICONTROL Bypass Single Sign-On]** 옵션을 선택하여 [사용자 역할](/help/marketo/product-docs/administration/users-and-roles/create-delete-edit-and-change-a-user-role.md)을(를) 이 제한에서 제외할 수 있습니다. 이렇게 하면 사용자가 정상적으로 로그인할 수 있습니다. 예를 들어 관리자는 로그인 화면을 통해 Marketo에 로그인해야 할 수 있습니다. SSO와 Universal ID가 모두 활성화되어 있으면 구독 간에 전환하려면 &quot;단일 사인온 우회&quot; 권한이 설정되어 있어야 합니다.

>[!CAUTION]
>
>새 사용자가 초대되면 초대 이메일을 받습니다. 그러나 **[!UICONTROL Require SSO]**&#x200B;을(를) 선택한 경우 **[!UICONTROL Bypass Single Sign-On]**(으)로 설정된 역할에 할당되지 않으면 해당 이메일을 받지 못합니다.

이제 모든 사용자(Single Sign-On을 우회할 수 있는 권한이 있는 사용자 제외)는 SSO 로그인만 사용할 수 있습니다.
