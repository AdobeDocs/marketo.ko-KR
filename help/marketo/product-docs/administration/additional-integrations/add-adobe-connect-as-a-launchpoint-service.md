---
unique-page-id: 7512270
description: Adobe Connect을 LaunchPoint 서비스로 추가하여 Marketo에서 웨비나 등록 및 출석을 관리합니다.
title: Adobe Connect as a [!DNL LaunchPoint] Service 추가
exl-id: 21beb156-6674-46a8-bdc5-345fcb4bb272
feature: Administration, Integrations
TQID: https://experienceleague.adobe.com/5FY-AYMAy60RGDnuFas2QP1PexqhIMohWh8egyllFNY
product_v2: id: b27e5950-9033-45ac-9f86-eb22e567f615
feature_v2: id: b13bd2ad-8e65-49e5-9691-2a0d31067b35id: d1d0a9cd-295d-4976-8c39-ddae266f240e
topic_v2: id: eddd9b14-83bd-4ff4-9072-54a4a484abb7
source-git-commit: a526f0bf4cbdf888b1c4462ba35dd2bc92316527
workflow-type: tm+mt
source-wordcount: 183
ht-degree: 7%

---

# Adobe Connect을 [!DNL LaunchPoint] 서비스로 추가 {#add-adobe-connect-as-a-launchpoint-service}

Marketo에서 Adobe Connect 웨비나 등록 및 출석을 관리합니다.

>[!NOTE]
>
>**관리자 권한 필요**

>[!NOTE]
>
>이 단계에서는 Adobe Connect 및 관리 권한에 대한 기존 구독이 필요합니다. 사용자 이름, 암호, 참가자 암호 및 모임 폴더 URL과 같은 설정을 사용할 수 있습니다.

>[!NOTE]
>
>Adobe Connect On Site는 현재 지원되지 않습니다.

1. **[!UICONTROL Admin]** 영역으로 이동합니다.

   ![](assets/add-adobe-connect-as-a-launchpoint-service-1.png)

1. **[!UICONTROL LaunchPoint]**&#x200B;를 클릭합니다.

   ![](assets/add-adobe-connect-as-a-launchpoint-service-2.png)

1. **[!UICONTROL New]** 드롭다운을 클릭하고 **[!UICONTROL New Service]**&#x200B;를 선택합니다.

   ![](assets/add-adobe-connect-as-a-launchpoint-service-3.png)

1. **[!UICONTROL Display Name]** 입력. **[!UICONTROL Service]**&#x200B;에서 Adobe Connect을 선택합니다.

   ![](assets/add-adobe-connect-as-a-launchpoint-service-4.png)

1. **[!UICONTROL Username]** 및 **[!UICONTROL Access Code]**&#x200B;을(를) 입력하십시오.

   ![](assets/add-adobe-connect-as-a-launchpoint-service-5.png)

1. **[!UICONTROL Meeting Folder URL]** 및 **[!UICONTROL Participant Password]**&#x200B;을(를) 입력하여 프로세스를 완료한 다음 **[!UICONTROL Create]**&#x200B;을(를) 클릭합니다.

   ![](assets/add-adobe-connect-as-a-launchpoint-service-6.png)

   >[!TIP]
   >
   >참가자 암호를 만들 때 이벤트에 대한 링크가 전송될 때 선택한 값이 쿼리 문자열에 포함됩니다. 고객 친화적으로 만드는 것이 좋습니다.

   >[!NOTE]
   >
   >Adobe Connect 계정에 로그인한 후 **[!UICONTROL Seminars]** 탭을 선택합니다. 특정 세미나를 자세히 살펴보지 않고 브라우저 표시줄의 URL을 **[!UICONTROL Meeting Folder URL]** 설정으로 복사합니다.

이제 Adobe Connect 서비스가 Marketo과 동기화됩니다!

![](assets/add-adobe-connect-as-a-launchpoint-service-7.png)

>[!MORELIKETHIS]
>
>[Adobe Connect으로 이벤트를 만드는 방법](/help/marketo/product-docs/demand-generation/events/create-an-event/create-an-event-with-adobe-connect.md){target="_blank"}을 알아보세요.
