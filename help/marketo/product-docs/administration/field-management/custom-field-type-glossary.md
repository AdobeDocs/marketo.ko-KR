...
>[!NOTE]
>
>대부분의 필드는 문자 수에서 최대치가 아니라 바이트 수만큼 최대치가 됩니다. 이러한 이유로 Marketo에서는 각 필드에 대한 명확한 문자 제한을 제공할 수 없습니다. 예외는 **문자열**&#x200B;이며, 최대 길이는 255자입니다.

## 부울 {#boolean}

**예제 이름:**&#x200B;은(는) 고객입니다. - 사용자를 고객으로 태그 지정
...
**예제 이름:** 대체 전자 메일 - 사용자를 위한 대체 전자 메일 주소를 유지합니다(실제 기본 전자 메일 주소 필드와 같이 이 필드에 전자 메일을 보낼 수 없음).
...
**예제 이름:** 구매 가능성 - 백분율 값 저장(CRM에서 계산)
...
**예제 이름:** 대체 전화 - 사용자를 위해 추가 전화 번호를 저장합니다.
...
**예제 이름:** 블로그 - 개인 블로그 URL을 저장할 필드를 만듭니다.
