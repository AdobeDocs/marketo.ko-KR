---
description: 이메일 열림을 캠페인별로 추적할지 또는 캠페인 간 사람당 한 번씩 추적할지 여부를 제어합니다.
title: 캠페인 수준 이메일 열람 추적
feature: Email Setup
exl-id: 23cd6ba8-5e3f-44f1-af8d-da03f4b038f2
TQID: https://experienceleague.adobe.com/kF-5V-G6-xr3uV-NQBTmFNfhQnk-r-RYt-v0HvJ-4Go
product_v2: id: b27e5950-9033-45ac-9f86-eb22e567f615
feature_v2: id: a7170d27-32ab-462b-a333-269abc654483id: d1d0a9cd-295d-4976-8c39-ddae266f240e
topic_v2: id: eddd9b14-83bd-4ff4-9072-54a4a484abb7
source-git-commit: a526f0bf4cbdf888b1c4462ba35dd2bc92316527
workflow-type: tm+mt
source-wordcount: 105
ht-degree: 20%

---

# 캠페인 수준 이메일 열람 추적 {#email-open-tracking-at-campaign-level}

이 기능을 사용하면 캠페인에서 열려 있는 각 이메일에 대해 한 번씩 또는 다른 캠페인에서 사용되는 횟수에 관계없이 각 이메일에 대해 한 번만 이메일 열기 추적을 제어할 수 있습니다.

>[!NOTE]
>
>**관리자 권한 필요**

1. **[!UICONTROL Admin]** 영역으로 이동합니다.

   ![](assets/email-open-tracking-at-campaign-level-1.png)

1. **[!UICONTROL Smart Campaign]**&#x200B;를 클릭합니다.

   ![](assets/email-open-tracking-at-campaign-level-2.png)

1. _전자 메일 열기 설정_ 옆에 있는 **[!UICONTROL Edit]**&#x200B;을(를) 클릭합니다.

   ![](assets/email-open-tracking-at-campaign-level-3.png)

1. 드롭다운을 클릭하고 원하는 설정을 선택한 다음 **[!UICONTROL Save]**&#x200B;을(를) 클릭합니다.

   ![](assets/email-open-tracking-at-campaign-level-4.png)

<table><tbody>
  <tr>
    <td><b>활성화됨</b></td>
    <td>이메일 열람은 각 캠페인에 대해 별도로 추적됩니다.</td>
  </tr>
  <tr>
    <td><b>비활성화됨</b></td>
    <td>이메일 열람수는 고유한 사용자 열람수만을 기준으로 계산됩니다.</td>
  </tr>
</tbody>
</table>
