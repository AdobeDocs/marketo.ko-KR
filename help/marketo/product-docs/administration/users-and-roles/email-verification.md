...
하나의 이메일 주소에 하나의 사용자 계정만 연결할 수 있습니다. 단일 이메일 주소와 연결된 사용자 계정이 여러 개 있는 경우 Marketo Engage은 충돌을 해결해야 하며 이메일 주소와 연결된 모든 사용자 로그인과 세 가지 해결 옵션을 표시합니다.<p>
`1` 현재 사용자 로그인 ID에 현재 이메일 사용<p>
`2` 현재 사용자 로그인 ID에 대해 새 이메일 사용<p>
`3` 다음 로그인까지 결정 지연
