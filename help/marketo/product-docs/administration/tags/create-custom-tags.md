...
description: 값을 사용하여 새 태그 유형을 만들고, 이 유형이 적용되는 프로그램 유형을 선택한 다음, 태그를 필수 또는 선택 사항으로 설정하는 방법입니다.
title: 사용자 정의 태그 만들기
...
1. 나열된 모든 프로그램 유형에 이 태그가 필요한지 여부를 결정합니다.
   ...
   >[!TIP]
   >
   >일부 프로그램 유형에는 비슷한 태그가 필요하지만 다른 유형에는 선택 사항인 태그를 만들려면 각각 &quot;[!UICONTROL Required]&quot;이(가) 선택된 프로그램 유형과 &quot;[!UICONTROL Required]&quot;이(가) 선택되지 않은 프로그램 유형에 적용되는 두 개의 태그를 별도로 설정해야 합니다.
