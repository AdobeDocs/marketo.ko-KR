...
세그먼트 탭에는 다양한 속성을 기반으로 설정하는 모든 사용자 지정 정의 세그먼트가 표시됩니다.  **세그먼트는 &#39;세그먼트 설정&#39; 페이지에 정의된 특정 기준을 충족하는 방문자 모음입니다.**  세그먼트는 특정 업계, 위치 또는 방문자의 온사이트 활동을 기반으로 하는 방문자가 될 수 있습니다.

웹 개인화에서 방문자는 두 개 이상의 세그먼트와 일치할 수 있습니다. 예를 들어 미국 방문자에 대한 세그먼트와 금융 회사에 대한 세그먼트가 있는 경우, 미국 은행의 웹 방문자는 **미국 방문자에 대한 세그먼트**&#x200B;와 금융 회사에 대한 세그먼트와 모두 일치합니다.

**그래프:**  세그먼트 페이지에는 세그먼트(y축)의 방문자 수와 세그먼트 이름(x축)의 방문자 수에 따라 선택한 세그먼트의 막대 차트 그래프가 표시됩니다.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colspan="1" rowspan="1"><p><strong>일치 항목</strong></p></td> 
   <td colspan="1" rowspan="1">세그먼트의 정의된 사용자 지정 기준을 충족하는 방문자 수</td> 
  </tr> 
  <tr> 
...
* 세그먼트 이름
* 세그먼트 생성 날짜
* 세그먼트와 함께 작동하는 캠페인을 보여주는 연결된 캠페인입니다. 반응 수를 클릭하면 세그먼트에 대한 캠페인 CTA(조치 호출)(Call to Action)를 표시하는 캠페인 페이지로 이동합니다
* 세그먼트에 대한 일치 항목(세그먼트 기준을 충족하는 방문자 수) 수와 세그먼트에 일치하는 별개의(고유) 방문자 수입니다. 고유 방문자 링크를 클릭하면 세그먼트 결과를 표시하는 방문자의 페이지로 이동합니다
* 세그먼트의 소유자/사용자 작성자
* 세그먼트와 연관된 도메인 사이트
* 세그먼트에서 선택한 기준의 요약

## 세그먼트 {#enable-or-disable-a-segment} 활성화 또는 비활성화

![](assets/image2014-11-12-10-3a48-3a9.png)

세그먼트를 활성화하거나 비활성화하려면 표에서 해당 세그먼트의 확인란을 선택하고 표 하단의 &quot;작업 선택&quot; 드롭다운 상자에서 &quot;활성화&quot; 또는 &quot;비활성화&quot; 동작을 선택합니다. 세그먼트가 비활성화되면 &quot;비활성화&quot;라는 단어가 [상태] 열 아래에 표시됩니다.

## 세그먼트 만들기 {#create-segments}

만든 세그먼트는 **세그먼트 설정** 페이지에서 정의한 특정 기준을 충족합니다. 캠페인에서 특정 대상을 타게팅하여 기준 조합을 기반으로 세그먼트를 사용자 지정할 수도 있습니다.

새 세그먼트를 만들려면

**세그먼트** 페이지에서 그래프에서 **새로 만들기**&#x200B;를 클릭합니다. 다음 화면이 나타납니다.
...
1. 
   ...
   * **타이밍:** 캠페인에 필요한 방문자 참여 수준을 정의합니다.

      * **시작 시**:방문자가 웹 사이트에 도달하는 즉시 참여
      * **1일 후 - 9번째 클릭**:웹 사이트에서 특정 클릭 수를 수행한 후 방문자에게 참여
...
* 다음 매개 변수 중에서 선택합니다.
   ...
   * **포함 **- 캠페인을 위치에 포함할지 제외할지를 선택합니다.
   * **추가할**   국가 선택 - 드롭다운 상자에서 세그먼트에 포함할 국가를 선택합니다. 국가 이름이 오른쪽에 나타납니다. 여러 국가를 선택할 수 있습니다.

국가가 추가되면 세그먼트의 시/도 및 우편 번호도 지정할 수 있습니다.

* **추가할**  주 또는 도를 선택합니다. 드롭다운 상자에서 포함시킬 미국 주 또는 캐나다 지역을 선택합니다. 여러 항목을 선택할 수 있습니다.
* **우편번호**  - 세그먼트에 포함할 우편번호를 입력합니다.
* **시**  - 포함시킬 도시 또는 도시를 입력합니다. 시 사이에 세미콜론을 사용합니다.

>[!TIP]
>
>**어떤 세그먼트 조건을 선택합니까? &#39;AND&#39; 또는 &#39;OR&#39;? **OR은 각 필드 내에서 추가 옵션으로 작동합니다. 잠재 고객은 세그먼트에 자격을 부여하기 위해 각 필드 내에서 선택한 여러 기준의 기준을 하나만 충족하면 됩니다. (예를 들어 잠재 고객은 미국 *또는 미국 산업계의* 중에서 선택할 수 있습니다.) AND는 이 세그먼트에 충족해야 하는 추가적인 필수 매개 변수로 작동합니다. (예를 들어 잠재 고객은 미국 및 국방 산업으로부터 모두 나와야 합니다.) 각 세그먼테이션 프로필 내에서 각 개별 필드는 선택한 세그먼트 조건에 따라 &quot;AND&quot; 또는 &quot;OR&quot;로 모두 작동할 수 있습니다.

**** 산업 분야 **프로필** 세그먼테이션 섹션에서 산업 **** 옆에 있는 상자를 선택합니다.
...
* 드롭다운 상자에서 다음 선택 항목 중에서 선택합니다.
   ...
   * Fortune 500대 기업 - 이 부문에 포춘지 500대 기업만 포함
   * Fortune 1000대 기업 - 이 부문에 Fortune 1000대 기업만 포함
...
>[!TIP]
>
>Job Title과 같은 Marketing의 모든 개인 데이터 필드에 따라 세그먼트 기준을 만듭니다.점수;역할;등..
>
>예. &quot;Job Title = CMO&quot; 및 &quot;점수가 50보다 작거나 같음&quot;

**Marketing to Email** CampaignSegment를 사용하여 Marketing To 이메일을 클릭하고 사이트에 도달하는 방문자의 이메일 참조를 통해 캠페인을 개인화합니다. 마케팅 프로그램 이름 또는 캠페인 이름별로 세그먼트화하고 이메일에서 웹으로 대화를 계속 진행합니다. 드롭다운에서 필드를 추가하려면 +를 선택합니다.

![](assets/image2015-5-27-17-3a20-3a34.png)

**상태**

잠재 고객의 상태에 따라 세그먼트를 정의합니다.알려진 항목 또는 익명 문서

* 

   * 알려진 방문자의 경우 드롭다운 상자에서 이 옵션을 선택합니다. 방문자는 웹 사이트에서 양식을 제출하고 웹 개인화 사람 페이지에 표시될 때 알려져 있습니다.
   * 익명 - 익명의 방문자를 위한 드롭다운 상자에서 이 옵션을 선택합니다.

![](assets/image2015-5-27-17-3a23-3a2.png)

### 행동 {#behavioral}

**방문 -** 방문자 행동에 따라 세그먼트를 정의하거나 식별합니다.
...
* 
   ...
   * 특정 방문 - 드롭다운 상자에서 이 옵션을 선택하여 특정 방문자를 지정합니다.

      * 오른쪽의 텍스트 상자에 추적할 방문자 번호를 입력합니다. 고유 웹 개인화 방문자 식별 번호는 방문자 페이지(방문자 페이지)를 클릭하고 오른쪽 패널에서 캠페인 설정을 클릭할 때 찾을 수 있습니다. 방문자 ID는 고급 설정 섹션에 있습니다. 방문자 ID는 URL에서도 찾을 수 있습니다(예: VISITOR=JZJIFJNUI60PZ8Y97BHTY9BL8PKWS).
...
* 
   ...
   * 검색된 방문자 - 드롭다운 목록에서 방문자 검색에서 추적할 용어를 선택하거나 자신의 검색어를 추가합니다. (검색어를 포함하는 구문을 포함하도록 기본값으로 설정되므로 검색 용어에 * 와일드카드가 필요하지 않습니다.)
...
**페이지**  제외 - 세그먼트에서 일치시키지 않으려는 특정 페이지를 제외합니다. 와일드카드로 * 사용이 허용됩니다.
...
타깃팅할 이벤트 값을 추가합니다. (예: 제3자 데이터 소스의 경우).
...
>[!TIP]
>
>**와일드카드 사용 - **검색어 내에 포함된 URL(예:&quot;[google.com](http://google.com)&quot; 또는 &quot;검색어 제품&quot;에서 이 필드를 와일드카드라고 하며 각 끝마다 별표(이 작은 남자*)로 입력해야 합니다. 따라서 [google.com](http://google.com)에서 오는 모든 내용은 * [google.com](http://google.com)*

## 세그먼트 편집 {#edit-segments}

만들어진 세그먼트를 편집할 수 있습니다.

1. 세그먼트를 편집하려면 **세그먼트**&#x200B;로 이동합니다.

   ![](assets/image2014-11-12-11-3a38-3a22.png)

1. **세그먼트** 테이블에서 편집할 세그먼트의 편집 아이콘( ![](assets/segment-edit.png))을 클릭합니다. 선택한 세그먼트가 있는 **세그먼트 설정** 페이지가 열립니다.
...
1. 삭제할 세그먼트의 삭제 아이콘( ![](assets/segment-delete.png) )** 클릭합니다.
1. **세그먼트**&#x200B;을(를) 삭제하려고 한다는 확인 메시지가 나타납니다.
