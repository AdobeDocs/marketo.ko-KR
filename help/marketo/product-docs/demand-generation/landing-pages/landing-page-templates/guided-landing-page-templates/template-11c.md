---
unique-page-id: 9437562
description: 템플릿 11C 안내 랜딩 페이지 템플릿에 대해 알아봅니다. 다운로드하여 Marketo 랜딩 페이지에 사용합니다.
title: 템플릿 11C
exl-id: 7398604e-5a38-4630-b28e-31dc16cfcec3
feature: Landing Pages, Guided Landing Page Templates
TQID: https://experienceleague.adobe.com/Zn4aDcuEFMO-x1oIWYaXo1b857B8ggq-W2oHjcHM8j0
product_v2:
  - id: b27e5950-9033-45ac-9f86-eb22e567f615
    internal-label: Marketo Engage
feature_v2:
  - id: f82558ea-6af5-44eb-a424-5b3389abb0a3
    internal-label: Templates
source-git-commit: 043d09315fec24d58897c362e9e28008df7d4aa9
workflow-type: tm+mt
source-wordcount: '105'
ht-degree: 3%
---
# 템플릿 11C {#template-11c}

[템플릿 11C 다운로드](https://51837-micrositemarketo.adobeio-static.net/lp-templates/template-11c.html)를 마우스 오른쪽 단추로 클릭하고 **다른 이름으로 링크 저장...**&#x200B;을 선택합니다.

>[!NOTE]
>
>[&#x200B; 템플릿을 다운로드하고 가져오는 방법에 대한 전체 단계는 여기에서 찾을 수 있습니다](/help/marketo/product-docs/demand-generation/landing-pages/landing-page-templates/guided-landing-page-template-list.md#how-to-import){target="_blank"}.

![](assets/image2015-8-4-14-3a3-3a44.png)

이 템플릿에는 다음 콘텐츠가 포함됩니다.

* 헤더(선택 사항)
* 영웅 이미지, 헤더, 비디오 및 본문 텍스트가 있는 기본 섹션
* 4개의 본문 섹션(선택 사항)
* 양식이 있는 바닥글(선택 사항)

**아래를 마우스 오른쪽 단추로 클릭하고 _다른 이름으로 링크 저장.._ 선택) 이 템플릿을 다운로드하려면:**

[템플릿 11C.html](https://51837-micrositemarketo.adobeio-static.net/lp-templates/template-11c.html)
