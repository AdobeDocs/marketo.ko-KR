...
생성 AI [응답 라이브러리](/help/marketo/product-docs/demand-generation/dynamic-chat/generative-ai/response-library.md)에서 제공하는 바로 가기는 방문자와 채팅할 때 도움을 받을 수 있는 효과적인 방법입니다. 텍스트 상자에 슬래시(`/`)를 입력하고 `ask` 또는 `search`을(를) 선택합니다.
