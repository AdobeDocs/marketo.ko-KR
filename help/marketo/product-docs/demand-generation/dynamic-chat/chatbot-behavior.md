...
익명의 잠재 고객이 있고 이메일 ID가 제공된 대화 중에 Dynamic Chat은 해당 이메일 ID에 알려진 잠재 고객 레코드가 있는지 확인하고 해당 레코드를 개인화에 실시간으로 사용합니다. 여러 레코드가 발견되는 경우 실시간으로 병합됩니다. 이 비헤이비어는 대화 및 대화 플로우 모두에 대해 구현됩니다.
