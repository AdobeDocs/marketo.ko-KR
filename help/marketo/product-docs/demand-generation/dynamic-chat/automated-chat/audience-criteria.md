---
description: Dynamic Chat 대화 상자를 표시하는 타깃팅을 위한 대상 기준에 대해 알아봅니다. 이벤트, 개인 및 회사 속성 및 우선 순위를 사용하여 대상을 정의합니다.
title: 대상자 기준
...
우선 순위는 잠재 고객이 두 개 이상의 자격이 있는 경우 받을 대화 상자를 결정합니다. 처음 [대화 상자를 만들](/help/marketo/product-docs/demand-generation/dynamic-chat/automated-chat/create-a-dialogue.md){target="_blank"}때 설정됩니다. 기존 대화 상자를 열고 대상 기준 탭에서 **[!UICONTROL Dialogue Details]**(으)로 이동하여 대화 상자의 우선 순위를 변경할 수 있습니다.

![](assets/audience-criteria-1.png)

## 이벤트 {#events}

이벤트를 사용하면 스크롤하는 양이나 페이지나 사이트에 있는 시간에 따라 방문자를 타깃팅할 수 있습니다. 아래 예에서 구성은 20초 이상 특정 페이지에 있었던 방문자를 타깃팅합니다.
...
선택할 _많은_ 특성 조합이 있습니다. 아래 예에서 구성은 50명 이상의 직원이 있는 회사에 근무하는 캘리포니아의 모든 알려진 사람을 타겟으로 합니다.
...
다음은 아직 데이터베이스에 없는 사람을 타깃팅합니다. 이 예에서 구성은 뉴욕 영역에 있는 모든 익명의 사용자를 타겟팅합니다.
...
1. **[!UICONTROL Inferred State]** 특성을 가져와서 _특성을 여기에 끌어다 놓기_&#x200B;로 끕니다.
   ...
   >[!NOTE]
   >
   >누군가가 웹 사이트를 방문하면 [Munchkin](/help/marketo/product-docs/administration/additional-integrations/add-munchkin-tracking-code-to-your-website.md){target="_blank"}에서 쿠키를 만들어 시스템에 넣습니다. IP 주소는 위치 및 기타 정보를 유추하기 위해 특수 데이터베이스에서 조회됩니다.
