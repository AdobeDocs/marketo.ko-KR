...
>[!NOTE]
>
>최근에 추가한 에이전트가 여기에 표시되지 않는 경우 Adobe Admin Console에서 추가한 후 최대 2시간이 걸릴 수 있습니다.
...
>[!TIP]
>
>사용자 지정 메시지를 만들 때 글꼴 스타일을 지정하고 링크를 사용하며 이모지를 삽입할 수도 있습니다.

## 설정 {#settings}

### 동시 라이브 채팅 제한 {#concurrent-live-chat}

에이전트가 한 번에 수행할 수 있는 동시 활성 채팅 수를 설정합니다. 1과 10 사이에서 설정할 수 있습니다.

![](assets/agent-management-10.png)

### 방문자 대기 시간 제한 {#visitor-wait-time}

방문자가 대체 메시지를 수신하기 전에 라이브 에이전트에 연결되기 위해 기다리는 최대 시간(초)을 제어합니다. 10초에서 500초 사이에서 설정할 수 있습니다.
