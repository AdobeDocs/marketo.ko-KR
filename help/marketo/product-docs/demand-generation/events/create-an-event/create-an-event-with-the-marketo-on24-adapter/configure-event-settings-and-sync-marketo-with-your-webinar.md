...
1. 이벤트 ID를 입력합니다(ON24에서 가져오기). 클릭 **저장**.
...
ON24 웨비나와 연결된 이벤트를 설정하면 이벤트 일정이 ON24의 데이터로 채워집니다. 이벤트 예약 대화 상자에 액세스하려면 다음 단계를 따르십시오.

1. 이벤트를 선택합니다. 다음을 클릭합니다. **이벤트 작업** 드롭다운 및 선택 **일정.**

   ![](assets/five.png)

1. 다음 항목 선택 **시작일, 종료일**, 및 **시간대**. 클릭 **저장**.
