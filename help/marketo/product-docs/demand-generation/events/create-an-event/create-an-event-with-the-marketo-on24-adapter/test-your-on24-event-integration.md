...
1. 을 사용하여 테스트 이름이 표시되는지 확인합니다. **등록됨** Marketo 이벤트의 멤버십 그리드의 상태입니다.
1. 테스트 이름도 로 표시되는지 확인합니다. **등록됨** ON24.
1. 테스트 이름을 등록하는 데 사용한 유효한 이메일 주소가 이벤트에 확인 이메일을 받았으며 고유 URL이 이메일에서 확인되었는지 확인합니다.

   >[!NOTE]
   >
   >다음을 사용해야 합니다. `{{member.webinar url}}` 각 등록자의 이메일에 고유 URL이 표시되도록 확인 이메일의 토큰 을 참조하십시오.
...
* Marketo과 ON24 간에 참석자 데이터가 동기화되면 Marketo은 멤버십 상태를 참석함, 온디맨드 참석함 또는 표시 없음으로 업데이트합니다. 의 경우 **요약** 탭에서 이벤트 상태가 다음으로 업데이트됩니다. **이벤트 완료**.
