...
>[!NOTE]
>
>확인 이메일을 이 고유 URL로 채우려면 이메일에 다음 토큰을 사용하십시오. `{{member.webinar url}}`. 확인 URL을 전송하면 이 토큰은 개인의 고유 확인 URL로 자동으로 확인됩니다.
>
>확인 이메일의 유형을 다음으로 설정 **운영** 등록을 하는 사람이 구독을 취소하더라도 확인 정보를 받을 수 있도록 합니다.

>[!TIP]
>
>확인, 미리 알림 또는 후속 이메일을 보내도록 ON24를 구성할 수 있습니다. 다음을 참조하십시오. [ON24 도움말 사이트](https://www.on24.com/live-webcast-elite/){target="_blank"} 추가 정보.
...
>[!CAUTION]
>
>어댑터가 작업을 수행하려면 등록 캠페인을 만들어야 합니다. 이 캠페인은 양식을 작성하는 사람에 의해 트리거되어야 하며 첫 번째 단계에서 해당 사람의 프로그램 상태를 (으)로 변경해야 합니다. **등록됨**. 그러면 캠페인에서 확인 이메일을 보냅니다. 자세한 내용은 이 문서의 나머지 부분을 참조하십시오.

**등록/확인(캠페인 트리거)**

* 스마트 목록
* 트리거 기준 **양식 작성**. 을 사용하여 양식이 있는 랜딩 페이지를 포함해야 합니다. **제한 추가**, 특히 동일한 양식을 여러 랜딩 페이지에서 사용하는 경우.
...
>[!NOTE]
>
>Marketo이 아닌 랜딩 페이지에서 Marketo 양식을 사용하는 경우 트리거는 다음과 같습니다. **양식 작성** (양식 이름 포함)

![](assets/image2015-12-22-15-3a20-3a51.png)

**플로우**

* **프로그램 상태 변경** - Webinar -> Registered로 설정합니다.

이 흐름 단계는 하위 캠페인을 설정할 때 첫 번째 흐름 단계로 필요합니다. 개인의 프로그램 상태가 등록됨으로 변경되면 Marketo에서 등록 정보를 ON24로 푸시합니다. 다른 상태는 사용자를 밀어내지 않습니다.

* **이메일 보내기** - 확인 이메일. 이 이메일을 다음으로 설정 **운영** 등록하지 않은 사람들이 계속 받게 됩니다.

다음 **이메일 보내기** 흐름 단계는 두 번째 단계여야 합니다. 확인 이메일에는 `{{member.webinar url}}`: ON24에서 Marketo으로 다시 전송된 정보로 채워집니다.

![](assets/image2015-12-22-15-3a29-3a50.png)

>[!NOTE]
>
>Marketo에서 작업이 수행되는 순서 때문에 이러한 흐름 단계의 순서가 중요합니다. 다음 **프로그램 상태 변경** 단계는 등록하기 위해 사람을 ON24로 보내고 고유 URL이 생성됩니다. 그런 다음 를 사용하여 이 고유한 URL이 포함된 확인 이메일을 보낼 수 있습니다. `{{member.webinar URL}}` 토큰.
>
>등록 오류가 있는 사람이 반환되면 이메일 확인을 받지 못합니다.

다음 단계는 [on24 이벤트 통합 테스트](/help/marketo/product-docs/demand-generation/events/create-an-event/create-an-event-with-the-marketo-on24-adapter/test-your-on24-event-integration.md){target="_blank"}.
