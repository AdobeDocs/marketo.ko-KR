...
* **ON24 웹캐스트 구독** - 현재 구독이 없는 경우 ON24에 직접 문의하십시오. **참고**: ON24 Hosted Edition이 필요합니다. ON24 이벤트 관리는 필요하지 않습니다.

* **ON24에 대한 관리자 권한** - 이 커넥터를 사용하고 ON24 시스템에서 게스트를 만들려면 이 커넥터가 필요합니다.
* **ON24 연결 자격 증명** - 통합을 활성화하려면 Marketo에 사용자 이름, 암호, 클라이언트 ID 및 클라이언트 키와 같은 정보를 입력해야 합니다. 자격 증명에 대한 도움이 필요한 경우 ON24 계정 관리자 또는 ON24 지원에 문의하십시오.
* **등록 양식** - 적절한 API와 함께 Marketo 양식 또는 Marketo이 아닌 양식을 사용하여 등록 데이터 및 등록자 정보가 Marketo에 전달되도록 합니다.
* **하위 캠페인 등록** - 이벤트 파트너 통합이 작동하려면 Marketo 이벤트의 등록 하위 캠페인을 올바르게 만들고 구성해야 합니다.
