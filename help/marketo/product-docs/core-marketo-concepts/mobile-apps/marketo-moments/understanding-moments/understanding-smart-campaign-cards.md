...
예약되었지만 아직 실행되지 않은 미래의 스마트 캠페인의 경우 Marketo 모멘트 카드에 아직 캠페인 통계가 표시되지 않습니다. 향후 릴리스에 표시될 예정입니다.
...
1. 여기에 스마트 캠페인의 흐름이 표시됩니다. 이 캠페인에는 하나의 흐름 단계만 있지만 여러 개의 흐름 단계가 있을 수 있습니다.
...
1. 맨 아래에 있는 두 점은 이 Smart Campaign에 연결된 두 개의 이메일이 있음을 나타냅니다. 다른 전자 메일에 대한 결과를 보려면 화면을 왼쪽으로 밉니다. 두 번째 이메일에 대한 결과는 다음과 같습니다.
...
테스트에 더 이상 필요하지 않은 카드가 있는 경우 왼쪽 또는 오른쪽으로 빠른 스와이프로 제거할 수 있습니다.
