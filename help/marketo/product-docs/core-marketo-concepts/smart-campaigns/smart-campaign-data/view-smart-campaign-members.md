---
unique-page-id: 1146937
description: Smart Campaign 구성원을 보는 방법에 대해 알아봅니다. 캠페인에 있는 사람과 흐름에서 이들의 상태를 확인합니다.
title: 스마트 캠페인 멤버 보기
exl-id: 027d93a9-f838-4f96-80d6-7bf6ee1cc03d
feature: Smart Campaigns
TQID: https://experienceleague.adobe.com/mNV2jG6Zyxj07lU-6lSc7kSVz0flF6xmVP5HxFECtDI
product_v2:
  - id: b27e5950-9033-45ac-9f86-eb22e567f615
feature_v2:
  - id: a7170d27-32ab-462b-a333-269abc654483
source-git-commit: a526f0bf4cbdf888b1c4462ba35dd2bc92316527
workflow-type: tm+mt
source-wordcount: 94
ht-degree: 10%

---

# 스마트 캠페인 멤버 보기 {#view-smart-campaign-members}

스마트 캠페인 플로우를 이미 통과한 사용자를 봅니다.

1. **[!UICONTROL Marketing Activities]** 으로 이동합니다.

   ![](assets/view-smart-campaign-members-1.png)

1. 스마트 캠페인에서 **[!UICONTROL View Campaign Members]**&#x200B;을(를) 클릭합니다.

   ![](assets/view-smart-campaign-members-2.png)

   >[!TIP]
   >
   >Smart Campaign 내의 어디에서든 캠페인 멤버를 볼 수 있습니다.

1. **[!UICONTROL Campaign Members]** 탭에는 스마트 캠페인 플로우를 이미 실행한 사용자가 표시됩니다.

   ![](assets/view-smart-campaign-members-3.png)

   >[!NOTE]
   >
   >스마트 캠페인이 어떤 사용자에서도 실행되지 않은 경우 캠페인 멤버 목록은 처음에 비어 있습니다.

   >[!MORELIKETHIS]
   >
   >[스마트 캠페인에서 차단된 사용자 보기](/help/marketo/product-docs/core-marketo-concepts/smart-campaigns/smart-campaign-data/view-blocked-people-in-a-smart-campaign.md){target="_blank"}
