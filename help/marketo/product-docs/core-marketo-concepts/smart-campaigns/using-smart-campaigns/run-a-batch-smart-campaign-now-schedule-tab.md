---
unique-page-id: 1146933
description: 지금 일괄 처리 스마트 캠페인 실행 | 예약 탭 - Marketo 문서 - 제품 설명서
title: 지금 일괄 처리 스마트 캠페인 실행 | 예약 탭
exl-id: a9014ab4-12a4-44ae-a361-72f95bb62e27
source-git-commit: 72e1d29347bd5b77107da1e9c30169cb6490c432
workflow-type: tm+mt
source-wordcount: '106'
ht-degree: 0%

---

# 지금 일괄 처리 스마트 캠페인 실행 | 예약 탭 {#run-a-batch-smart-campaign-now-schedule-tab}

일괄 처리 캠페인 빌드가 완료되면 지금 또는 나중에 실행하도록 선택할 수 있습니다. 바로 실행하는 방법은 다음과 같습니다.

1. 일괄 처리 스마트 캠페인을 선택하고 **예약** 탭을 클릭하고 **한 번 실행**.

   ![](assets/runcampaignnow-hands.png)

1. 확인 **지금 실행** 을(를) 선택하고 을(를) 클릭합니다. **실행**.

   ![](assets/image2014-9-19-15-3a57-3a4.png)

1. 을 클릭하여 확인 **실행** 한 번만 더

   ![](assets/image2014-9-19-15-3a57-3a19.png)

   진정해! 다음을 수행할 수도 있습니다 [나중에 실행 예약](/help/marketo/product-docs/core-marketo-concepts/smart-campaigns/using-smart-campaigns/schedule-a-batch-smart-campaign-to-run-later.md) 원한다면

   >[!NOTE]
   >
   >* [나중에 실행하도록 배치 스마트 캠페인을 예약합니다.](/help/marketo/product-docs/core-marketo-concepts/smart-campaigns/using-smart-campaigns/schedule-a-batch-smart-campaign-to-run-later.md)
   >* [반복 배치 캠페인 예약](/help/marketo/product-docs/core-marketo-concepts/smart-campaigns/using-smart-campaigns/schedule-a-recurring-batch-campaign.md)

