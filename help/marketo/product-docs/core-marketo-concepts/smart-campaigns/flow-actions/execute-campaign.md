---
description: Campaign 실행 흐름 단계를 사용하는 방법을 알아봅니다. 누군가 이 단계에 도달하면 다른 스마트 캠페인을 실행하십시오.
title: 캠페인 실행
...
API](https://experienceleague.adobe.com/en/docs/marketo-developer/marketo/rest/assets/smart-campaigns#batch)에서 [ 일정 또는 요청 캠페인을 사용할 때 둘 다 내 토큰에 대한 값을 전달할 수 있도록 해줍니다. 이는 호출하는 캠페인의 해당 토큰에 대해 설정된 값을 무시합니다. 그런 다음 Campaign이 다른 캠페인을 실행하고 &quot;상위 컨텍스트 사용을 True로&quot; 설정하면 애플리케이션에 설정된 값이 아니라 API를 통해 전달된 값이 사용됩니다.
