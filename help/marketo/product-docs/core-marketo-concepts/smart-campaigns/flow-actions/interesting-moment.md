---
unique-page-id: 1146956
description: 흐름 단계에서 흥미로운 순간을 만드는 방법을 알아봅니다. CRM에서 판매 가시성에 대한 주요 작업을 기록합니다.
title: 즐거운 순간
exl-id: fa1a45b8-1e62-497e-9441-bf6859addb42
feature: Smart Campaigns
source-git-commit: 60c5603fa29bb1039b9d477633beb2c6f5c63486
workflow-type: tm+mt
source-wordcount: '108'
ht-degree: 3%

---

# 즐거운 순간 {#interesting-moment}

Marketo Sales Insight이 있는 경우 **관심 있는 순간** 흐름 단계를 사용하여 직원들이 수행하는 멋진 작업에 대한 영업 팀의 가시성을 제공할 수 있습니다.

![](assets/interesting-moment-1.png)

1. 사용할 **[!UICONTROL type]** 즐거운 순간을 선택하세요.

   ![](assets/interesting-moment-2.png)

1. 영업팀에 즐거운 순간을 설명하는 **[!UICONTROL description]**&#x200B;을(를) 만듭니다.

   ![](assets/interesting-moment-3.png)

   >[!TIP]
   >
   >_적은 값이_&#x200B;보다 큽니다. 영업 팀과 협력하여 흥미로운 순간이 실제로 흥미로운지 확인합니다.

[관심 있는 순간에 토큰](/help/marketo/product-docs/marketo-sales-insight/msi-for-salesforce/features/tabs-in-the-msi-panel/interesting-moments/trigger-tokens-for-interesting-moments.md){target="_blank"}을 사용하여 매우 유용한 동적 설명을 만들 수도 있습니다.

>[!MORELIKETHIS]
>
>* [관심 있는 순간 사용](/help/marketo/product-docs/marketo-sales-insight/msi-for-salesforce/features/tabs-in-the-msi-panel/interesting-moments/using-interesting-moments.md){target="_blank"}
>* [즐거운 순간을 위한 토큰](/help/marketo/product-docs/marketo-sales-insight/msi-for-salesforce/features/tabs-in-the-msi-panel/interesting-moments/trigger-tokens-for-interesting-moments.md){target="_blank"}
