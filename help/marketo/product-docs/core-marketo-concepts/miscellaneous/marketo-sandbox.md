...
프로덕션 인스턴스와 이미 동기화된 경우 Marketing To 샌드박스를 일반 CRM과 동기화할 수 없습니다. 동기화하려면 CRM의 샌드박스를 사용하고 원래 동기화된 모든 단계[와 동일한 모든 단계를 따릅니다.](http://docs.marketo.com/display/DOCS/CRM+Sync)
...
* 사용자를 추가하려는 경우 이 프로세스는 [프로덕션](http://docs.marketo.com/display/DOCS/Managing+Marketo+Users#ManagingMarketoUsers-CreateUsers)에서 사용자 추가와 같습니다. 이미 Marketing To 로그인이 있는 경우 다른 이메일 주소를 사용해야 합니다.
* Marketing To 샌드박스는 비어 있게 시작되지만 프로덕션 인스턴스와 동일한 기능을 사용할 수 있습니다.
* 샌드박스를 채우기 위해 1회 인스턴스 복사를 요청하는 지원 사례를 제출할 수 있습니다. 그러나 인스턴스 복사본은 *모든*&#x200B;을(를) 가져오지 않습니다. 자세한 내용은 마케팅 지원에 문의하십시오.
* 샌드박스에서 프로그램을 만들고 프로덕션으로 이동하려는 경우 [프로그램 가져오기](http://docs.marketo.com/display/DOCS/Import+a+Program)를 수행할 수 있습니다.
