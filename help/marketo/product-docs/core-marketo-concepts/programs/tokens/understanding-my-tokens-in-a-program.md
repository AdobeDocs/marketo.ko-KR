...
description: 프로그램에서 내 토큰 이해 - Marketo 문서 - 제품 설명서
...
토큰은 이메일, 랜딩 페이지 및 스마트 캠페인에 사용하여 보다 쉽게 작업할 수 있는 변수입니다.

내 토큰 외에도 프로그램에서 기본 제공 토큰을 사용할 수도 있습니다. 다음을 확인하십시오 [토큰 개요](/help/marketo/product-docs/demand-generation/landing-pages/personalizing-landing-pages/tokens-overview.md).

## 내 토큰  {#my-tokens}

내 토큰은 누구나 만들 수 있는 사용자 지정 변수입니다. 그렇습니다 [생성됨](/help/marketo/product-docs/core-marketo-concepts/programs/tokens/managing-my-tokens.md) campaign 폴더 또는 프로그램에서 사용할 수 있습니다.
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td>이 토큰을 사용하여 다음을 수행하십시오 <a href="/help/marketo/product-docs/email-marketing/general/functions-in-the-editor/create-a-calendar-event-ics-file.md">달력 이벤트 파일 추가(.i</a><a href="/help/marketo/product-docs/email-marketing/general/functions-in-the-editor/create-a-calendar-event-ics-file.md">cs)</a> 을 이메일 및 랜딩 페이지에 추가합니다.</td> 
  </tr> 
  <tr> 
   ...
   <td>이 토큰은 날짜 값을 보유합니다. 날짜는 연(예: 2016-05-23)으로 표시됩니다.</td> 
  </tr> 
  <tr> 
   ...
   <td>이 토큰을 사용하여 이메일에서 Velocity 스크립트를 실행합니다. 추가 정보 <a href="https://developers.marketo.com/documentation/email-scripting/" title="팔로우 링크" rel="nofollow">여기</a>. </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td>이 토큰은 <a href="/help/marketo/product-docs/core-marketo-concepts/smart-campaigns/flow-actions/use-tokens-in-flow-steps.md">점수 흐름 단계 변경</a>. </td> 
  </tr> 
  <tr> 
   ...
   <td colspan="1">이 토큰을 사용하면 Marketo 프로그램의 일부가 되는 리드를 어떤 SFDC Campaign도 추가하는지에 추가할 수 있습니다.</td> 
  </tr> 
  <tr> 
   ...
   <td>그냥 문자요 HTML이 오버킬 때 사용합니다. 텍스트 토큰의 크기 제한은 524,288자(UTF-8) 또는 2MB입니다.</td> 
  </tr> 
 </tbody> 
</table>
...
>[!CAUTION]
>
>Microsoft Dynamics 또는 Salesforce의 Sales Insight에서 이메일을 보낼 때 내 토큰이 확인되지 않습니다. 표준 토큰만 채워집니다(리드, 회사 등). 토큰의 기본값 _will_ 하지만,

## 토큰 중첩 {#nesting-tokens}

새 토큰을 만들면 트리의 다른 개체에서 참조할 수 있습니다. 관리를 쉽게 하기 위해 토큰을 생성한 위치에 대한 이름 지정 구조가 있습니다.

* **로컬 토큰:** 토큰이 해당 프로그램 또는 폴더에서 바로 만들어졌습니다.
* **상속된 토큰:** 토큰이 상위 수준 프로그램 또는 폴더의 트리 위로 생성되었습니다.
* **재정의된 토큰:** 토큰이 상속된 후 이 프로그램 또는 폴더에서 예외가 발생했습니다.

전역 변수를 만든 다음 트리의 낮은 수준에서 재정의할 수 있습니다.

프로그램 및 폴더를 이동하면 토큰도 영향을 받습니다. 이동하는 동안 참조가 끊기지 않았는지 항상 확인합니다.

>[!NOTE]
>
>참여 프로그램에서 보내는 이메일이 기본 프로그램의 하위 이메일(참여 프로그램에 로컬이 아님)인 경우 이메일에 사용된 모든 내 토큰은 하위 이메일이 있는 기본 프로그램에서 해결됩니다.
