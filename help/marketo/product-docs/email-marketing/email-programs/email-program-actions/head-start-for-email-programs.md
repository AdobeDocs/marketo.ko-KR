...
1. 예약 타일에서 이메일을 예약한 다음 **헤드 스타트** 상자.

   ![](assets/three-1.png)

   헤드 시작을 선택하면 프로그램이 예약된 시간보다 약 12시간 전에 처리를 시작합니다. 처리가 시작되면 프로그램이 잠깁니다.
...
1. 클릭 **프로그램 승인**.
   ...
   * **실행 대기 중:** 프로그램 승인 후.
   * **처리가 시작되어 다음 실행을 기다리는 중:** 처리가 진행 중입니다.
   * **처리가 완료되어 실행을 기다리는 중:** 처리가 완료되었습니다. 이제 이메일을 통해 예정된 시작 시간을 기다리고 있습니다.
   * **완료됨:** 프로그램이 완료되었습니다.

   >[!TIP]
   >
   >프로그램이 잠긴 후 이메일을 보내기 전에 취소하시겠습니까? 문제 없습니다! 간단히 클릭 **프로그램 중단** 승인 타일의 오른쪽 하단에 있습니다.
...
기존의 헤드 스타트 기능을 사용하려면 최소 12시간 전에 프로그램을 예약해야 합니다. 수신자 시간대는 무엇을 의미합니까? 수신자 시간대가 활성화된 경우 가장 이른 시간대(UTC +14:00)의 자정에 이메일 프로그램을 시작합니다. 따라서 활성화하려면 **모두** 시작 및 수신자 시간대, 프로그램을 예약해야 함 **가장 이른 시간대(UTC +14:00)보다 최소 12시간 빠름**.)

즉, 미국/로스앤젤레스에 있는 사용자가 헤드 스타트 및 수신자 시간대를 모두 활성화하려면 프로그램을 예약해야 합니다 **34시간** 미리. 어떻게 우리가 이 번호를 알게 됐죠?

![](assets/image2017-12-5-13-3a11-3a46.png)

[자세히 알아보기](/help/marketo/product-docs/email-marketing/email-programs/email-program-actions/scheduling-with-recipient-time-zone/schedule-email-programs-with-recipient-time-zone.md) 받는 사람 시간대로 전자 메일 프로그램을 예약하는 방법에 대한 정보입니다.
