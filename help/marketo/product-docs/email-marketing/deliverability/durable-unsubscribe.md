...
Marketo은 &quot;지속적&quot;으로 만들기 위해 구독 취소 기능의 비헤이비어를 개선했습니다. 개인 세부 정보 레코드에 표시되는 구독 취소 플래그와 별개인 마스터 이메일 상태가 추가되었습니다.
...
한 명 이상의 사용자에 대해 아래와 같이 **[!UICONTROL Change Data Value]** 흐름 단계를 실행합니다.

![데이터 값 흐름 변경 단계](assets/durable-unsubscribe-3.png)

## 새 사용자 만들기 {#creating-a-new-person}

새 사용자가 생성되면 Marketo은 마스터 이메일 상태 테이블에 대해 확인합니다. 이전에 구독을 취소한 경우 레코드가 구독 취소로 업데이트됩니다.
...
구독을 취소하면 동일한 이메일 주소를 가진 모든 사람이 구독 취소되는 것처럼, 다시 구독하면 동일한 이메일 주소를 가진 모든 사람이 다시 구독됩니다.
