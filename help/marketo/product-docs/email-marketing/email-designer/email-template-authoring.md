---
description: 재사용을 위해 독립 실행형 이메일 템플릿을 만드는 방법을 알아봅니다. 이메일 Designer에서 맞춤형 템플릿을 구축하여 더 빠른 디자인과 일관성을 보장합니다.
title: 이메일 템플릿
...
>[!NOTE]
>
>이메일 Designer의 이메일 템플릿은 이메일 Designer에서 이메일을 작성하는 데만 사용할 수 있습니다. 클래식 이메일 편집기에서 참조할 수 없습니다.
...
>[!NOTE]
>
>이메일 Designer에는 이미지만 추가할 수 있으며 현재 다른 파일 유형은 추가할 수 없습니다.
...
토큰은 클래식 편집기에서 작동하는 것과 동일한 방식으로 이메일 Designer에서 작동하지만 아이콘은 다르게 보입니다. 아래 예에서는 대체 텍스트가 있는 이름 토큰을 추가하는 방법을 간략하게 설명합니다.
...
1. 원하는 토큰을 찾고 **..** 아이콘을 클릭합니다(+ 아이콘을 클릭하면 대체 텍스트가 없는 토큰이 추가됨).
   ...
   >[!NOTE]
   >
   >&quot;대체 텍스트&quot;는 기본값을 나타내는 이메일 Designer 용어입니다. 예: ``{{lead.First Name:default=Friend}}``. 선택한 필드에 사용자에 대한 값이 없는 경우 사용하는 것이 좋습니다.
...
>[!MORELIKETHIS]
>
>[전자 메일 작성](/help/marketo/product-docs/email-marketing/email-designer/email-authoring.md){target="_blank"}
