...
조각은 하나 이상의 이메일 및 이메일 템플릿에서 참조할 수 있는 재사용 가능한 구성 요소입니다. 일반적으로 만들고 프로젝트에 신속하게 삽입할 수 있는 콘텐츠 블록(텍스트, 이미지 또는 둘 다)입니다. 이 기능을 사용하면 여러 사용자 지정 콘텐츠 블록을 사전 빌드하여 이메일 콘텐츠를 취합함으로써 디자인 프로세스를 향상시킬 수 있습니다. 일반적인 사용 사례에는 이메일, 이벤트 초대 배너, 시즌 메시지 등에 대한 머리글/바닥글 콘텐츠 블록이 포함됩니다.
