...
아래 예에서는 3개의 열로 구성된 구조 구성 요소 내에서 패딩 및 수직 정렬을 조정하는 방법을 간략하게 설명합니다.

이메일에서 직접 구조 구성 요소를 선택하거나 왼쪽 메뉴에서 사용할 수 있는 탐색 트리를 사용합니다.

도구 모음에서 열 선택 을 클릭하고 편집할 열을 선택합니다. 구조 트리에서 선택할 수도 있습니다.

해당 열에 대해 편집 가능한 매개변수가 스타일 탭에 표시됩니다.



정렬(Alignment)에서 위쪽(Top), 중간(Middle) 또는 아래쪽(Bottom)을 선택합니다.



패딩에서 모든 면에 대한 패딩을 정의합니다.

패딩을 미세 조정하려면 각 면에 대해 다른 패딩을 선택합니다. 동기화를 중단하려면 잠금 아이콘을 클릭합니다.



다른 열의 정렬 및 패딩을 조정하려면 유사하게 진행합니다.
