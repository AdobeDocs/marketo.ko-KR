...
description: Azure에 앱을 등록하여 Dynamics 동기화용 클라이언트 ID 및 앱 ID를 가져오는 방법에 대해 알아봅니다. 인증에 Azure Active Directory 및 앱 등록을 사용합니다.
...
Azure Active Directory가 온-프레미스 디렉터리를 클라우드로 확장하여 온-프레미스 ADFS 인증을 사용하는 [!DNL MS Dynamics 365] CRM을 지원합니다.
...
1. 관리자 권한이 있는 계정을 사용하여 Microsoft Azure 관리 포털에 [로그인](https://login.microsoftonline.com/){target="_blank"}합니다. 왼쪽 탐색 창에서 **[!UICONTROL Admin]** 항목을 확장하고 **[!UICONTROL Azure AD]**&#x200B;을(를) 선택하여 Office 365 관리 센터를 통해 Microsoft Azure 포털에 액세스할 수도 있습니다.
   ...
   >[!NOTE]
   >
   >Azure 계정이 없는 경우 [등록](https://azure.microsoft.com/en-us/free/){target="_blank"}할 수 있습니다. 자세한 내용은 Microsoft 설명서 를 참조하거나 Microsoft 담당자에게 문의하십시오. Azure 계정을 만든 후에는 아래 설명된 절차를 사용하여 하나 이상의 앱을 등록할 수 있습니다.
   >
   >
   >Azure 계정이 있지만 Azure 구독에서 [!DNL Microsoft Dynamics 365]을(를) 가진 [!DNL Office 365] 구독을 사용할 수 없는 경우 [다음 지침](https://msdn.microsoft.com/office/office365/howto/setup-development-environment#bk_CreateAzureSubscription){target="_blank"}에 따라 두 계정을 연결하세요.
