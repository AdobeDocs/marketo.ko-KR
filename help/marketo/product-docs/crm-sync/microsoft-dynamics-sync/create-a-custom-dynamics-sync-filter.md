...
description: 사용자 정의 Dynamics 동기화 필터 만들기 - Marketo 문서 - 제품 설명서
...
Dynamics CRM의 모든 것을 Marketo에 동기화하지 않으려는 경우 걱정 마! Marketo을 사용하면 동기화 필터를 설정하고 기록의 일부만을 동기화할 수 있습니다.
...
>[!NOTE]
>
>데이터베이스나 Marketo이 아니라 Dynamics CRM에서 이러한 변경 작업을 수행해야 합니다.

Marketo은 자동 백그라운드 동기화 도중 이 필드를 찾고 이 로직을 기반으로 동기화할 레코드를 결정합니다.
...
>[!CAUTION]
>
>Marketo에서 레코드를 건너뛰도록 하는 유일한 방법은 필드 값을 **No**&#x200B;로 명시적으로 설정하는 것입니다. Marketo은 필드 값이 비어 있어도 여전히 레코드를 동기화합니다.
...
1. **표시 이름** 필드에 **SyncToMkto**&#x200B;를 입력하고 **2개의 옵션**&#x200B;을 **데이터 유형**&#x200B;으로 선택합니다. 그런 다음 **저장 및 닫기**&#x200B;를 클릭합니다.
   ...
   >[!NOTE]
   >
   >비동기 작업 과정이 설정된 경우 레코드는 필드에서 설정한 기본 SyncToMkto 값을 가져오고 작업 흐름 실행이 끝나면 몇 초 후에 올바른 값을 가져옵니다. 기본값이 Yes로 설정된 경우 해당 레코드는 Marketo에서 생성된 다음 오래된 상태가 됩니다. 이를 방지하려면 **No**&#x200B;을 기본값으로 사용합니다.

1. 이 프로세스를 반복하여 연락처, 계정, 기회 및 사용자 지정 엔터티 등 동기화를 제한하려는 다른 개체에 대해 **SyncToMkto** 필드를 만듭니다.

## Marketo {#select-the-filter-in-marketo}에서 필터 선택

초기 동기화를 완료한 경우에도 Marketo과 동기화할 필드를 선택합니다.
...
좋아요. Marketo에 대한 동기화 필터를 활성화했습니다.
...
1. 동기화 작업 시작
   ...
   **SyncToMkto** 값이 **No**&#x200B;에서 **Yes**&#x200B;로 변경되면 Dynamics는 Marketo에 즉시 이 레코드 동기화를 시작하도록 알립니다. 레코드가 이미 있으면 Marketo에서 업데이트합니다. 그렇지 않으면 Marketo에서 레코드를 만듭니다.

   >[!TIP]
   >
   >이 경우 Marketo 로그에 `Create [StartSync]` 작업이 추가됩니다.

1. 동기화 작업 중지

   레코드가 SyncToMkto 값을 [예]에서 [아니요]로 변경하면 Marketo에 이 레코드 동기화를 중지하라는 알림이 표시됩니다. 하지만 레코드는 삭제되지 않고 대신 업데이트 받기를 중단하고 오래된 상태가 됩니다.
