...
Marketing to는 Salesforce의 필드 정보를 동기화합니다. 자세한 내용

## 어떤 필드가 동기화됩니까? {#which-fields-are-synced}

SFDC의 대부분의 표준 필드와 동기화 사용자가 볼 수 있는 권한이 있는 모든 사용자 정의 필드를 동기화합니다.

## Marketing to의 레코드가 Salesforce의 리드 또는 담당자인지 어떻게 판단합니까? {#how-do-you-determine-if-a-record-in-marketo-is-a-lead-or-a-contact-in-salesforce}

SFDC 유형이라고 하는 시장이 있습니다 세 가지 가능한 값이 있습니다.리드, 연락처 또는 비어 있음 비어 있는 경우 이 Marketing To 리드가 SFDC에 존재하지 않음을 의미합니다.

## 리드 또는 연락처가 SFDC에서 삭제되었는지 어떻게 확인할 수 있습니까? {#how-do-you-determine-if-a-lead-or-contact-is-deleted-in-sfdc}

SFDC isDeleted라는 필드가 Markto에 있습니다. 값이 true이면 리드가 SFDC에서 삭제됩니다.

## SFDC에서 추가한 새 필드가 Marketing To에도 추가되도록 하려면 어떻게 합니까? {#how-do-i-make-sure-a-new-field-i-add-in-sfdc-also-gets-added-to-marketo}

>[!TIP]
>
>두 시스템 모두에서 필드를 만들려면 먼저 SFDC에서 만들면 Marketing에 자동으로 동기화됩니다.

SFDC에서 새 필드를 추가하고 동기화 사용자에게 해당 필드를 볼 수 있는 권한이 있으면 자동으로 Marketing에 추가됩니다.
...
필드 유형을 변경하면 필드가 일치하지 않으면 데이터가 삭제됩니다(하지만 먼저 경고가 표시됨). 데이터를 유지하려면 필드 유형을 변경한 후 데이터를 내보내고 다시 가져와야 합니다.
...
공식 필드는 동기화되지만 [시스템 모드 스탬프에 대한 업데이트가 있을 때까지 수식에 있는 참조에 대한 업데이트는 동기화되지 않습니다](https://help.salesforce.com/apex/HTViewSolution?id=000193203&amp;language=en_US).
