...
Marketo에는 SFDC Type이라는 필드가 있습니다. 여기에는 리드, 연락처 또는 비어 있는 세 가지 값이 있습니다. 비어 있는 경우 이 Marketo 리드가 SFDC에 존재하지 않음을 의미합니다.
...
필드 유형을 변경하면 Marketo은 필드의 데이터가 일치하지 않는 경우(하지만 먼저 경고가 표시됨) 해당 필드의 데이터를 삭제합니다. 데이터를 보존하려면 데이터를 내보내고 필드 유형을 변경한 후 다시 가져옵니다.
