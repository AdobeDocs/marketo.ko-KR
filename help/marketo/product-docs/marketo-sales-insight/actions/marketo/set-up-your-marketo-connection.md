...
>[!NOTE]
>
>Sales Insight Actions를 프로비저닝하면 Marketo이 자동으로 자격 증명을 작업에 보내고 인스턴스를 Marketo에 연결합니다. 이 단계는 필수입니다 **연결이 설정되어 있지 않은 경우에만** Sales Connect 인스턴스가 프로비저닝되면 연결이 설정되면 Marketo 관리 설정 페이지에 입력한 자격 증명과 연결 끊기 옵션이 표시됩니다.
