...
**문제 해결 단계:** 이에 대한 모든 인스턴스를 [지원 팀](https://nation.marketo.com/t5/Support/ct-p/Support).
...
**발생한 결과:** 액세스 또는 새로 고침 토큰이 만료되었습니다. 토큰 만료 기준 [salesforce의 세션 설정](https://salesforce.stackexchange.com/questions/10759/invalid-grant-expired-access-refresh-token-error-when-authenticating-access-via).
...
**범주:** 액세스/유효성 검사\
**메시지:** 필드를 생성/업데이트할 수 없음: MSE_Replaced__c. 이 필드의 보안 설정을 확인하십시오.
**발생한 결과:** 가장 최근 활동 로깅 트랜잭션을 수행하는 데 필요한 Sales Insight Actions 사용자 정의 필드에 대한 쓰기 액세스 권한이 사용자에게 없습니다. 팀이 패키지를 설치했지만 사용자에 대한 올바른 필드를 활성화하지 않았습니다.\
