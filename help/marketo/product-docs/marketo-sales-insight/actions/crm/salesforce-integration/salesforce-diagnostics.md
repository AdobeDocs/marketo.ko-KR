...
**문제 해결 단계:** Salesforce 관리자가 사용자 API 액세스 권한을 부여해야 합니다.
...
**진행 중인 작업:** 승인 진행 중인 레코드에서 승인을 가진 사용자가 승인하거나 거부할 때까지 추가 편집 내용이 잠깁니다.\
**문제 해결 단계:** 위의 내용을 참조하십시오.

**오류:** EXPIRED_ACCESS
**범주:** 인증
**메시지:** 잘못된 부여: 만료된 액세스/새로 고침 토큰
**진행 상황:** 액세스 또는 새로 고침 토큰이 만료되었습니다. 토큰은 Salesforce의 [세션 설정](https://salesforce.stackexchange.com/questions/10759/invalid-grant-expired-access-refresh-token-error-when-authenticating-access-via)을 기반으로 만료됩니다.
**문제 해결 단계:** 다시 인증해야 합니다. Salesforce 연결을 끊고 다시 연결합니다.

**오류:** FAILED_WRITE\
**범주:** 간헐적\
**메시지:** 파일의 끝에 도달했습니다.\
**현재 상황:** Salesforce에서 고객 측의 최적 상태가 아닌 트리거로 인해 성능 문제가 발생할 수 있습니다.\
**문제 해결 단계:** 다시 시도 논리에서 이 작업을 처리해야 합니다. 그래도 작동하지 않는 경우 Salesforce 관리자에게 문의하여 문제 있는 트리거를 해결하십시오.
...
**진행 상황:** 업데이트 시 Salesforce의 기존 잘못된 데이터가 강제 적용됩니다.\
**문제 해결 단계:** 위의 내용을 참조하십시오.
...
**범주:** 인증\
**메시지:** 잘못된 부여: 비활성 조직\
**진행 상황:** Salesforce 조직이 더 이상 활성화되지 않습니다.
**문제 해결 단계:** 연결을 끊은 다음 Salesforce에서 다시 연결합니다.

**오류:** 비활성 사용자
**범주:** 인증
**메시지:** 잘못된 부여: 비활성 사용자
**해결 방법:** Salesforce 사용자가 더 이상 활성화되지 않음
**문제 해결 단계:** 연결을 끊은 다음 Salesforce에서 다시 연결합니다.

DELETE **오류:** INSERT_UPDATE_MAINTENANCE_NOT_ALLOWED_DURING_MAINTENANCE\
**범주:** 간헐적\
**메시지:**(추가 메시지 없음)\
**진행 상황:** Salesforce 인스턴스가 유지 관리 모드에 있습니다.\
...
**문제 해결 단계:** Salesforce에서 사용자에게 액세스 권한을 부여하거나 해당 사용자에 대해 해당 개체에 대한 최근 활동 로깅을 사용하지 않도록 설정하십시오.
...
**현재 상황:** 사용자가 가장 최근 활동 로깅 트랜잭션을 수행하는 데 필요한 Sales Insight Actions 사용자 지정 필드에 대한 쓰기 액세스 권한이 없습니다. 팀이 패키지를 설치했지만 사용자에 대한 올바른 필드를 활성화하지 않았습니다.\
**문제 해결 단계:** Salesforce 관리자가 사용자 지정 필드에 대한 액세스 권한을 부여하거나 가장 최근 활동 로깅을 해제해야 합니다.

**오류:** INVALID_GRANT\
**범주:** 인증\
**메시지:** 잘못된 권한 부여: ip 제한\
**문제 발생:** Salesforce에 액세스하려고 하지만 IP 제한 사항이 있어서 액세스할 수 없습니다.\
**문제 해결 단계:** Salesforce 관리자가 IP를 테스트하려면 허용 목록에 추가하다가 필요합니다. 사용자는 지원 팀에 연락하여 IP 주소를 받아야 합니다.
...
**문제 해결 단계:** Salesforce의 잠재 고객 개체에 대한 읽기 및 업데이트 액세스 권한을 부여하거나 잠재 고객 레코드에 대한 전자 메일 로깅 및 가장 최근 활동 로깅을 해제하십시오.

**오류:** QUERY_TIMEOUT\
**범주:** 간헐적\
**메시지:** 쿼리 요청이 너무 오래 실행되고 있습니다.\
**진행 상황:** 위의 내용을 참조하십시오.\
**문제 해결 단계:** 다시 시도 논리에서 이 작업을 처리해야 합니다. 그래도 작동하지 않는 경우 Salesforce 관리자와 협력하여 문제가 있는 트리거를 해결하십시오.
...
**범주:** 간헐적\
**메시지:** 서버 사용 중\
**현재 상황:** Salesforce에서 고객의 최적 상태가 아닌 트리거로 인해 성능 문제가 발생할 수 있음\
**문제 해결 단계:** 다시 시도 논리에서 이 작업을 처리해야 합니다. 그래도 작동하지 않는 경우 Salesforce 관리자와 협력하여 문제 트리거를 해결하십시오.
...
**진행 중:** Salesforce에서 처리되지 않은 예외.\
**문제 해결 단계:** Salesforce에 사례를 기록하고 오류 메시지의 숫자 값을 복사합니다. 오류를 제대로 처리하지 않는 Salesforce 코드입니다.
