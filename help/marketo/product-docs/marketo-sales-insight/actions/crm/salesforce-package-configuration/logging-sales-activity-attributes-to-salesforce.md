...
1. 아래 표를 기반으로 추가할 필드에 해당하는 데이터 유형을 선택하고 을(를) 클릭합니다. **다음**.

1. 추가할 필드에 해당하는 필드 이름과 레이블을 입력합니다.

아래 표의 각 열에 대한 설명:

* **필드 레이블**: UI에 표시되는 필드 이름(팀에서 가독성을 향상하도록 이 이름을 사용자 지정할 수 있음)
* **필드 이름**: 필드의 기술적 이름(입력한 필드 이름이 아래 표의 필드 이름과 일치하는지 확인)
* **API 이름**: API 필드의 기술적 이름(입력한 API 이름이 아래 표의 API 이름과 일치하는지 확인)
* **데이터 유형**: 필드 유형
* **크기**: 텍스트 필드의 크기
